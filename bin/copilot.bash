...
# Command copilot runs the GitHub Copilot CLI agent in a Docker container using
# runsc. The current working directory is mounted into the container. Copilot
# configuration is stored in the directory ${XDG_DATA_HOME}/copilot-docker.
...
function _main() {
    ...
    COPILOT_IMAGE=${COPILOT_IMAGE:-"ghcr.io/ianlewis/copilot"}

    XDG_DATA_HOME="${XDG_DATA_HOME:-${HOME}/.local/share}"
    COPILOT_DATA_HOME="${XDG_DATA_HOME}/copilot-docker"
    ...
    verified_sha=$(cosign verify-attestation \
        ...
        "${COPILOT_IMAGE}" | jq -r '.payload' | base64 -d | jq -r '.subject[0].digest.sha256')

    # Ensure we have the latest image
    docker pull "${COPILOT_IMAGE}@sha256:${verified_sha}"
